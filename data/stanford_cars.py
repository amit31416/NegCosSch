...
from data.stanford_cars_dataset2 import CarsDataset2
#import data.stanford_cars_dataset as StanfordCars
from torchvision.datasets.folder import default_loader
from torch.utils.data import Dataset

#from config import car_root, meta_default_path

meta_default_path = '/fs/nexus-scratch/amit314/datasets/stanford_cars/devkit/cars_{}.mat'
car_root = '/fs/nexus-scratch/amit314/datasets/stanford_cars/'
...
def get_scars_datasets(train_transform, test_transform, train_classes=range(120),
    ...
    labels_meta = mat_io.loadmat(car_root+"devkit/cars_meta.mat")
    #print(labels_meta)

    train_dataset_whole = CarsDataset2(data_dir=car_root, transform=TwoCropTransform(train_transform), train=True)
    
    #train_dataset_whole = MyStanfordCars(root=car_root, split='train',transform=TwoCropTransform(train_transform),download=True)
    train_dataset_whole = subsample_classes(train_dataset_whole, include_classes=train_classes)
    #print(len(set(train_dataset_whole.target)))
    ...
    test_dataset_known = CarsDataset2(data_dir=car_root, transform=test_transform, train=False)
    
    #test_dataset_known = MyStanfordCars(root=car_root, split='test',transform=test_transform,download=True)
    test_dataset_known = subsample_classes(test_dataset_known, include_classes=train_classes)
    #print(len(set(test_dataset_known.target)))
    #print(len(test_dataset_known))
    # Get testset for unknown classes
    test_dataset_unknown = CarsDataset2(data_dir=car_root, transform=test_transform, train=False)
    #test_dataset_unknown = MyStanfordCars(root=car_root, split='test',transform=test_transform,download=True)
    test_dataset_unknown = subsample_classes(test_dataset_unknown, include_classes=open_set_classes)
    #print(len(set(test_dataset_unknown.target)))
