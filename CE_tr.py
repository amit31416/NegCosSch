...
import json 
import argparse
from set_default_params import set_default_forCE, set_filename_forCE
...
def fetch_args():
  ...
  # model
  parser.add_argument('--encoder_lossname', type=str, default='CE',help="SupCon|SimCLR")
  ...
  parser.add_argument('--temp_scheduler', type=str, default='gcosm', help = 'gcosm')
  parser.add_argument('--T', type=int, default=200)
  parser.add_argument('--shift', type=float, default=1.0, help='shift for gcos= 0,0.25,0.5,0.75,1')
  parser.add_argument('--Tp', type=float, default=2.0)


  args = parser.parse_args()
  args = set_default_forCE(args)
  print(args)
  return args
...
def test_linear(model,testloader,from_softmax = False):
      ...
      for idx,(x,y,idd) in enumerate(testloader):
        ...
        with torch.set_grad_enabled(False):
          ...
          f_x=predictions.data.cpu().numpy()
          p_x =max_logit 
        ...
        label_correct = correct if label_correct is None else np.concatenate((label_correct,correct),axis=0)
        # statistics
        running_corrects += np.sum(f_x==y)
      
      prob_osr= list(p) #max_freq_counter(f,p,y_list)
...
def get_performance_OSR_CE(model,train_loader,test_loader,out_loader,from_softmax =False):

   print("Max logit based inference: -----")

   acc_test,probOSR_test,y_list,predicted_label=test_linear(model,test_loader, from_softmax =from_softmax)
   #print("out")
   _,probOSR_out,_,_=test_linear(model,out_loader, from_softmax =from_softmax)
...
def train_CE(model,train_loader,test_loader,out_loader,args):
    ...
    for epoch in range(args.s_epoch,args.epochs+1):
      ...
      if(epoch % args.PRINT_INTERVAL ==0):
         ...
         results=get_performance_OSR_CE(model,train_loader,test_loader,out_loader)
...
if __name__ == '__main__':  
  ...
  model_filename, result_filename = set_filename_forCE(args)
  ...
  train_CE(model,train_loader,test_loader,out_loader,args)
  ...
  if(args.dataset in ['cub','aircraft','scars']):
      ...
      print(inference_method)
      results=get_performance_OSR_CE(model,train_loader,test_loader,out_loader_easy)
      print(results)
      all_results[inference_method]=results
